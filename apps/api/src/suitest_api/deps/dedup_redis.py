"""FastAPI dependency: Redis client used for webhook dedup SETNX (M1d-16/17/18).

The GitLab/GitHub webhook receivers de-duplicate ``(project_id, commit_sha,
trigger)`` tuples; the Jira webhook receiver deduplicates
``(workspace_id, issue_key, changelog_id)`` tuples — both inside a short TTL
window via Redis ``SETNX``. We piggy-back on the existing ARQ pool
