...
- **Blocker #3 — per-workspace `public_id`: `test_cases` DONE (commit `4adb927`, migration `0037`: added `workspace_id` + composite unique `(workspace_id, public_id)` + a `before_insert` derivation listener); the real-backend e2e now creates a case end-to-end. STILL OPEN for `runs`/`requirements`/`defects`** — same global-unique-vs-per-workspace bug; apply the identical pattern (each already has or needs `workspace_id`; `defects` already has the column). Do `runs` next (blocks journey step 6 "Run now" in 2nd+ workspaces). Apply the dev migration with: `set -a; . ./.env; set +a; uv run python -c "from alembic import command; from alembic.config import Config; c=Config('packages/db/alembic.ini'); c.set_main_option('script_location','packages/db/alembic'); c.set_main_option('sqlalchemy.url',__import__('os').environ['SUITEST_DATABASE_URL']); command.upgrade(c,'head')"` (`make migrate` is broken from repo root — relative `script_location`).
