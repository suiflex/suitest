...
- **Blocker #3 — per-workspace `public_id`: `test_cases` (migration `0037`) + `runs` (migration `0038`) DONE. STILL OPEN: `requirements`, `defects`** — same fix; `defects` already has a `workspace_id` column (constraint swap only), `requirements` needs the column + a derivation listener (mirror `_run_workspace_id` in `public_id.py`, deriving from `project_id`). Apply the dev migration with: `set -a; . ./.env; set +a; uv run python -c "from alembic import command; from alembic.config import Config; c=Config('packages/db/alembic.ini'); c.set_main_option('script_location','packages/db/alembic'); c.set_main_option('sqlalchemy.url',__import__('os').environ['SUITEST_DATABASE_URL']); command.upgrade(c,'head')"` (`make migrate` is broken from repo root — relative `script_location`).
