...
|-------|------|-------|-------|
| Trigger (workspace LLM provider, web UI) | `none` / not set | `ollama` / `llamacpp` / `vllm` / `lmstudio` | `anthropic` / `openai` / `gemini` / `groq` / `openrouter` / `azure` / `bedrock` / `vertex` / `deepseek` / `chatgpt` (Sign in with ChatGPT — see §3) / `google-vertex` / `google-codeassist` (Sign in with Google — see §3) / `antigravity` (Sign in to Antigravity — see §3) / `mock` (test/dev only — see §3) |
