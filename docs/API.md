...
| POST | `/generators/prd` | **(M3-6)** LLM extracts user stories from a PRD → DRAFT cases (agentic steps). SSE. **409 `LLM_NOT_CONFIGURED`** when no active LLM; `LLM_OUTPUT_TRUNCATED` error frame when the model hits its output limit (8192 tokens) before returning cases | **Yes (CLOUD/LOCAL)** |
| POST | `/generators/mcp-discovery` | **(M3-9)** LLM explores a registered MCP provider's tool catalog → DRAFT contract cases. SSE. **409** when no active LLM; **404** unknown provider; `EMPTY_CATALOG` error frame when no tools; `LLM_OUTPUT_TRUNCATED` error frame when the model hits its output limit (8192 tokens) before returning cases | **Yes (CLOUD/LOCAL)** |
| POST | `/generators/url-semantic` | **(M3-7)** LLM decomposes an intent ("checkout flow") on a URL → FE_WEB journey cases (playwright-mcp). SSE. **409** when no active LLM; `LLM_OUTPUT_TRUNCATED` error frame when the model hits its output limit (8192 tokens) before returning cases | **Yes (CLOUD/LOCAL)** |
