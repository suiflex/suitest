...
- [x] **M3-2b** Sign in with ChatGPT as a second auth method beside the pasted key — [`chatgpt_oauth.py`](../packages/core/src/suitest_core/chatgpt_oauth.py) (device-code + PKCE protocol) + [`chatgpt_oauth_service.py`](../apps/api/src/suitest_api/services/chatgpt_oauth_service.py) (flow state, token refresh) + 4 routes under `/llm-config/chatgpt/login`; `llm_configs.auth_method` / `oauth_tokens_encrypted` (migration `0049`); FE auth-method switch in `LlmSettingsPanel.tsx`. The `api_key` credential mode (token exchange → real OpenAI key) is complete; **`subscription` mode stores and refreshes the credential but is not yet routed** — the ChatGPT backend speaks the Responses API while `LiteLLMProvider` maps `openai/*` onto `/chat/completions`, and the 8 `get_provider` call sites each resolve credentials themselves. Needs a live spike + a shared credential resolver before it can run.
