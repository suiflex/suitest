...
**Run pipeline (journey steps 6–7) is REAL and FUNCTIONAL.** Drove a real ZERO run end-to-end against the live stack (api `:4000` + `make dev-runner` + Redis): `POST /runs` → ARQ → `run_test_case` → `playwright-mcp` spawned via `npx -y @playwright/mcp@latest` → `browser_navigate` tool invoked → run reached a terminal status + `run_steps` persisted. A pre-existing dev workspace even has a real 5-step **PASS** run. **Remaining to green a saucedemo run:** the FIRST `playwright-mcp` invoke downloads the browser and exceeds the 30s MCP tool timeout (`MCP_TOOL_TIMEOUT: browser_navigate ... 30.0s`) — pre-install the browser or raise the tool timeout; then lock "Run now" through the UI (the run-detail UI + WS streaming are already built). S3/MinIO (`SUITEST_S3_ENDPOINT` is configured) is only needed once steps emit screenshots.
