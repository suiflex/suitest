...
- Real-backend e2e (iteration 2): `make e2e-real` → `apps/api/scripts/seed_zero_e2e.py` (an EMPTY `e2e-zero` workspace + a runnable `e2e-run` workspace) + `apps/web/playwright.realbackend.config.ts` boots ZERO api (`make dev-api-zero`) + web + the ARQ runner, and drives `e2e/realbackend/{bootstrap,run}.spec.ts` through the live stack (no `/api` mocks). **Journey steps LOCKED in the e2e: 1 (login), 2 (create project + suite), 4 (author a manual case), 5 (search), 6 (Run now), 7 (live results → PASS), 8 (analytics dashboard), 11 (ZERO hides AI panel + AI tab).** Step 3 (MCP test-connection) UI verified (live test needs a real `playwright-mcp`). **Remaining: 9 (mark a suite gating), 10 (make it fail → triage → defect).**
