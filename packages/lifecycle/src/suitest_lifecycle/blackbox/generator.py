...
def _case(
    n: int, title: str, desc: str, category: str, prio: Priority, ref: str, steps: list[tuple[str, str]]
) -> PlanCase:
    return PlanCase(
        id=f"TC{n:03d}",
        title=title,
...
def generate_cases(
    discovery: DiscoveryResult, cfg: BlackboxUiConfig
...
''',
    ...
    crawled = [
        ...
        if p.pattern not in ("login", "not_found") and not p.protected
    ]
...
def export_blackbox_tests(
    ...
    *,
    ...
    if llm is not None and prd_context:
        ...
        pairs += prd_cases(
            discovery, cfg, llm, prd_context, existing, start_n=len(pairs)
        )
